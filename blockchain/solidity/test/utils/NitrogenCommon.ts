...
export async function deployNitrogenV2Common(token: string, isOldMPT: boolean = false) {
...
export function deployNitrogenWithUSDTAndOldPool() {
    return deployNitrogenV2Common(ethMainnetBetaConfig.USDT_ADDRESS, true);
}
...
export async function deployMoleculaPool() {
    ...
    const USDT = await ethers.getContractAt('IERC20', ethMainnetBetaConfig.USDT_ADDRESS);
    return { moleculaPool, poolOwner, poolKeeper, malicious, USDT, randomAccount };
}

export async function initNitrogenForPause() {
    const { moleculaPool, randAccount, guardian, poolOwner } = await deployNitrogenWithUSDT();
    const USDT = await ethers.getContractAt('IERC20', ethMainnetBetaConfig.USDT_ADDRESS);
...
export async function getRidOf(
    ...
    if (!(await moleculaPool.connect(poolOwner).isInWhiteList(token))) {
        await moleculaPool.connect(poolOwner).addInWhiteList(token);
    }
