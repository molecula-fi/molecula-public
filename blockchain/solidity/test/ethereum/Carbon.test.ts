...
describe('Test Carbon', () => {
    describe('General solution tests', () => {
        ...
        it('Test update oracle', async () => {
            const { supplyManager, agentLZ, moleculaPool, mockLZEndpoint, lzMessageDecoder } =
                await loadFixture(deployCarbon);
            ...
            expect(quote.nativeFee).to.not.equal(0n);
            // update oracle
            await agentLZ.updateOracle({ value: quote.nativeFee });
            // decode message
            const res = await lzMessageDecoder.decodeUpdateOracle(
                await mockLZEndpoint.lastMessage(),
            );
            expect(res.totalValue).to.equal(220_000_000_000_000_000_000n);
            expect(res.totalShares).to.equal(INITIAL_SUPPLY);
        });
        it('LZ Deposit and Redeem Flow', async () => {
            ...
            await wmUSDT.connect(poolKeeperSigner).approve(await agentLZ.getAddress(), totalRedeem);
            ...
            expect(await wmUSDT.totalSupply()).to.equal(0n);
