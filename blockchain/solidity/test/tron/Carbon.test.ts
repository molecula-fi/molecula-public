...
//         await accountantLZ.setUnderlyingToken(await moleculaToken.getAddress());
...
//             const { moleculaToken, owner, accountantLZ } = await loadFixture(deployVaultSolution);
//             expect(moleculaToken.owner()).to.exist;
//             expect(await moleculaToken.owner()).to.equal(owner!.address);
//         });
//         it('Success Deposit', async () => {
//             const { moleculaToken, oracle, user, mockLZEndpoint, accountantLZ } =
