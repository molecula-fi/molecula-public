 ...
 * @dev Abstract contract that implements pause functionality for mint and burn operations.
 */
abstract contract PausableIIssuer is PausableContract {
    /// @dev Function selector for mint operations from IIssuer interface.
    bytes4 internal constant _MINT_SELECTOR = IIssuer.mint.selector;

    /// @dev Function selector for burn operations from IIssuer interface.
    bytes4 internal constant _BURN_SELECTOR = IIssuer.burn.selector;

    /// @dev Initializes the contract by registering mint and burn functions
    ///      as pausable operations.
    constructor() {
